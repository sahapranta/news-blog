...
import { ArticleProps, CategoryProps } from '@/types/model';
...
const CategoryPage: React.FC<{ category: CategoryProps; articles: PaginatedResponse<ArticleProps> }> = ({ category, articles }) => {
    const Heading = (
        <div className="mb-6">
            <h1 className="mb-6 border-b-4 border-red-600 pb-4 text-3xl font-bold text-gray-900">{category.title}</h1>
            <p className="text-lg text-gray-600">{category.description || 'এই বিভাগে ইসকনের বিভিন্ন কার্যক্রম ও সংবাদসমূহ অন্তর্ভুক্ত রয়েছে।'}</p>
        </div>
    );
    return (
        <HomeLayout hasSidebar heading={Heading}>
            <Head title={`${category.title} সংবাদ - মাসিক হরেকৃষ্ণ সমাচার`}>
                <meta
                    name="description"
                    content={`${category.title} সংবাদ - ${category.description || 'এই বিভাগে ইসকনের বিভিন্ন কার্যক্রম ও সংবাদসমূহ অন্তর্ভুক্ত রয়েছে।'}`}
                />
                <meta property="og:title" content={`${category.title} সংবাদ - মাসিক হরেকৃষ্ণ সমাচার`} />
                <meta
                    property="og:description"
                    content={category.description || 'এই বিভাগে ইসকনের বিভিন্ন কার্যক্রম ও সংবাদসমূহ অন্তর্ভুক্ত রয়েছে।'}
                />
            </Head>
            ...
            {articles.data.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 text-center text-gray-500">
                    <Folder className="mb-6 h-20 w-20 text-red-300" />
                    ...
                    <Link
                        href="/categories"
                        className="mt-6 inline-block rounded-full bg-red-600 px-6 py-2.5 text-sm font-semibold text-white transition duration-300 hover:bg-red-700"
                    >
                        সব বিভাগে ফিরে যান
                    </Link>
                </div>
