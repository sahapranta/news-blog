...
const CommentSection: React.FC<CommentSectionProps> = ({ id }) => {
  ...
  const handleEdit = async (id: string | number, newBody: string) => {
    setComments(prev =>
      prev.map(comment =>
        comment.id === id ? { ...comment, body: newBody, saving: true } : comment
      )
    );
    ...
    try {
      ...
      setComments(prev =>
        prev.map(comment =>
          comment.id === id ? { ...data.data, saving: false } : comment
        )
      );
      ...
      setComments(prev =>
        prev.map(comment =>
          comment.id === id ? { ...comment, saving: false } : comment
        )
      );
    }
  };
        ...
        {loading
          ...
          : comments.map((comment) => {
            if (comment.user.id === auth?.user?.id) {
              return <CommentCardEditable key={comment.id} comment={comment} user={auth.user} onEdit={handleEdit} onDelete={handleDelete} />
            } else {
